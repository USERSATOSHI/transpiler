...
export const $httpRequest: FunctionData = {
    name: "$httpRequest",
    brackets: true,
    optional: false,
    ...
    description:
        "creates a http request to the given url with the given method and returns the response",
    ...
    code: (data: funcData, scope: Scope[]) => {
        ...
        if ($httpRequest.brackets) {
            if (
                !data.total.startsWith($httpRequest.name + "[") &&
        ...
        {
            currentScope[ 'packages' ] += "const UNDICI = await import('undici');\n";
        }
