...
class LoadCommands {
    ...
    async load(client, path, debug = true) {
        ...
        async function walk(file) {
            const something = await fs_1.default.promises
                .readdir(file, { withFileTypes: true })
                .then((f) => {
                return f.map((d) => {
                    d.name = `${file}${path_1.default.sep}${d.name}`;
                    return d;
                });
            });
        ...
        if (typeof path !== "string")
            throw new TypeError(`Expecting typeof string on 'path' parameter, get '${typeof path}' instead`);
        ...
        for (const { name } of dirents) {
            ...
            for (const cmd of cmds) {
                ...
                if (!("type" in cmd))
                    cmd.type = "basicCommand";
                const valid = validCmds.some((c) => c === cmd.type);
                if (!valid) {
                    debugs.push(`|${this.colors.typeError?.command || ""}${cmd.name || cmd.channel}${this.allColors.reset}|${this.colors.typeError?.type || ""}${cmd.type} ${this.allColors.reset}|${this.colors.typeError?.text || ""}Invalid Type Provided${this.allColors.reset}|`);
                    continue;
                }
                cmd.load = true;
                cmd.__path__ = name.split(path_1.default.sep).slice(-2).join(path_1.default.sep);
                cmd.__uglify__ = this.uglify;
                const Cmd = new command_1.Command(cmd);
                try {
                    ...
                    console.error(e);
                    debugs.push(`|${this.colors.failLoad?.command || ""}'${cmd.name || cmd.channel}'| ${this.colors.failLoad?.type || ""}${cmd.type}${this.allColors.reset}| ${this.colors.failLoad?.text || ""}Failed To Load${this.allColors.reset}|
|-------------------------------------------|`);
                    continue;
                }
...
|------------------------------------------|`);
        ...
        if (debug) {
            console.log(`|  ${this.colors.loaded?.command || ""}Command${this.allColors.reset}  |  ${this.colors.loaded?.type || ""}Type${this.allColors.reset}  |  ${this.colors.loaded?.text}State${this.allColors.reset}  |
|------------------------------------------|\n` + debugs.join("\n"));
        }
    }
